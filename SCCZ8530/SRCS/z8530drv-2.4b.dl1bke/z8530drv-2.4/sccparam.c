	...
	if (argc < 4)
	{
		if (param < 0x80)
		{
			...
			if (is_socket)
			{
				...
				error = ioctl(fd, TIOCSCCGKISS, &cmd);
		
			if (error < 0) 
				handle_error(error, "TIOCSCCGKISS", argv[2]);
			if (cmd.param == NO_SUCH_PARAM) 
				handle_error(-ENODATA, "TIOCSCCGKISS", argv[2]);
		...
		if (param >= 0x80)
		{
			if (is_socket)
			{
				...
				error = ioctl(fd, TIOCSCCGSTAT, &stat);
			...
			memcfg.rxbuffers = stat.rxbuffers;
			memcfg.txbuffers = stat.txbuffers;
			memcfg.bufsize   = stat.bufsize;
			
			switch(param)
			{
				case C_RXBUFFERS:
					memcfg.rxbuffers = val;
					break;
				case C_TXBUFFERS:
					memcfg.txbuffers = val;
					break;
				case C_BUFSIZE:
					memcfg.bufsize = val;
					break;
			...
			if (is_socket)
			{
				...
				error = ioctl(fd, TIOCSCCSMEM, &memcfg);
			
			if ( error < 0) handle_error(error, "TIOCSCCSMEM", "");
		} else {		
			cmd.command = param;
			cmd.param = val;
			...
			if (is_socket)
			{
				...
				error = ioctl(fd, TIOCSCCSKISS, &cmd);

			if (error < 0) handle_error(error, "TIOCSCCSKISS", argv[2]);
		}
	}
