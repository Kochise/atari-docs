#define RCS_ID "$Id: scc.c,v 1.66 1996/10/05 17:46:45 jreuter Exp jreuter $"

#define BANNER "Z8530 SCC driver version 2.4b.dl1bke (experimental) by DL1BKE\n"
...
/*
   ...
   ******************************************************************** 
   ...
   961120	- Will hopefully compile with kernel 2.1.x again

   970115	- fixed return codes and added a missing scc_enqueue_buffer()
   		  in scc_net_tx(). Will compile with 2.1.20 again.
   ...
   NB -- if you find errors, change something, please let me know
   	 ...
   	 "v2.4b.dl1bke" with your own. Just to avoid confusion...
...
scc_ioctl(struct scc_channel *scc, unsigned int cmd, void *arg)
{
	...
	if (!Driver_Initialized)
	{
		if (cmd == TIOCSCCCFG)
		{
			...
			if (!arg || copy_from_user(&hwcfg, arg, sizeof(hwcfg)))
				return -EFAULT;
	...
	if (!scc->init)
	{
		...
		{
			if (!suser()) return -EPERM;
			if (!arg || copy_from_user(&scc->modem, arg, sizeof(struct scc_modem)))
				return -EFAULT;
	...
	switch(cmd)
	{
		...
		case TIOCCHANMEM_OLD:
			if (!suser()) return -EPERM;
			if (!arg || copy_from_user(&memcfg, arg, sizeof(struct scc_mem_config)))
				return -EINVAL;
		...
		case TIOCSCCSTAT_OLD:
			if (!arg || copy_to_user(arg, &scc->stat, sizeof(struct scc_stat)))
				return -EFAULT;
			return 0;
		
		case TIOCSCCGKISS:
		case TIOCGKISS_OLD:
			if (!arg || copy_from_user(&kiss_cmd, arg, sizeof(struct scc_kiss_cmd)))
				return -EINVAL;
		
			kiss_cmd.param = kiss_get_param(scc, kiss_cmd.command);
			return copy_to_user(&kiss_cmd, arg, sizeof(struct scc_kiss_cmd))? -EFAULT:0;
			break;
...
scc_tty_ioctl(struct tty_struct *tty, struct file * file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case TIOCMGET:
			...
			return put_user(result,(unsigned int *) arg);
		...
		case TIOCMSET:
			switch (cmd) 
			{
				...
				case TIOCMBIC:
					...
					break;
				case TIOCMSET:
					if ((error = get_user(value, (unsigned int *) arg)) < 0)
						return error;
		...
		case TCGETS:
			...
			if (error)
				return error;
			if (!arg || copy_to_user((void *) arg, scc->tty->termios, sizeof(struct termios))) 
				return -EFAULT;
			return 0;
		...
		case TCSETSW:		/* should wait 'till flush, but... */
			if (!arg || copy_from_user(scc->tty->termios, (void *) arg, sizeof(struct termios)))
				return -EFAULT;
...
scc_write(struct tty_struct *tty, int from_user, const unsigned char *buf, int count)
{
	...
	while (cnt2)
	{
		...
		{
			down(&scc_sem);
			if (copy_from_user(scc_wbuf, buf, cnt))
			{
				up(&scc_sem);
				return -EFAULT;
			}
...
scc_net_tx(struct sk_buff *skb, struct device *dev)
{
	...
	if (bp == NULLBUF || skb->len > scc->stat.bufsize)
	{
