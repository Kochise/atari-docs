...
int main(int argc, char **argv)
{
	...
	if (fd < 0)
	{
		if ((fd = open("/dev/scc0", O_RDWR)) < 0)
		{
			perror("FATAL: Can't open /dev/scc0: ");
	...
	while (!feof(fp))
	{
		...
		{		
			...
			if (is_socket)
			{
				...
				if (strchr(val, '/') == NULL)
					sprintf(fname, "/dev/%s", val);
				else
					strcpy(fname, val);
			
				if ((fd=open(fname, O_RDWR)) < 0)
				{
					fprintf(stderr,"Can't open %s",val);
					perror("");
					continue;
				}
			}
		...
		if (fd)
		{
			if (hwinit)
			{
				...
				switch (command)
				{
					...
					case I_RXBUFFERS:
						memcfg.rxbuffers = (unsigned int) param;
						break;
						
					case I_TXBUFFERS:
						memcfg.txbuffers = (unsigned int) param;
						break;
					...
					case I_WARN:
						fprintf(stderr, "Warning: parameter 'slip' now unsupported\n");
						break;
